...
def model_confusion_experiment(models_path, device='cpu'):
    sdn_name = 'cifar10_resnet56_sdn_ic_only'
    cnn_name = 'cifar10_resnet56_cnn'
    ...
    sdn_layer_correct, sdn_layer_wrong, sdn_instance_confusion = mf.sdn_get_confusion(sdn_model,
                                                                                      loader=dataset.test_loader,
                                                                                      confusion_stats=confusion_stats,
                                                                                      device=device,
                                                                                      save_name=sdn_name + "_test")
    sdn_layer_correct, sdn_layer_wrong, sdn_instance_confusion = mf.sdn_get_confusion(sdn_model,
                                                                                      loader=dataset.train_loader,
                                                                                      confusion_stats=confusion_stats,
                                                                                      device=device,
                                                                                      save_name=sdn_name + "_train")
    sdn_correct_confusion, sdn_wrong_confusion = get_sdn_stats(sdn_layer_correct, sdn_layer_wrong,
                                                               sdn_instance_confusion)

    cnn_correct, cnn_wrong, cnn_instance_confidence = mf.cnn_get_confidence(cnn_model,
                                                                            loader=dataset.test_loader,
                                                                            device=device)
    ...
    # corrects and wrongs
    af.overlay_two_histograms(sdn_images, '{}_corrects_wrongs'.format(sdn_name), sdn_correct_confusion,
                              sdn_wrong_confusion, 'Correct', 'Wrong', 'SDN Confusion')
    af.overlay_two_histograms(cnn_images, '{}_corrects_wrongs'.format(cnn_name), cnn_correct_confidence,
                              cnn_wrong_confidence, 'Correct', 'Wrong', 'CNN Confidence')
