...
# python train_networks.py -d $dataset -a $arch -t sdn_ic -s $seed --head_arch conv $pooling --size_after_pool $size_after_pool --stacking --detach_prev --heads all --save_test_logits --skip_train_logits --tag "${seed}_${dataset}_${arch}_${pooling}_heads_ztw" &
wait
for head_id in $(seq 0 $heads); do
    python train_networks.py -d $dataset -a $arch -t running_ensb -s $seed --head_arch conv $pooling --size_after_pool $size_after_pool --save_test_logits --skip_train_logits --stacking --detach_prev --head_ids $head_id --alpha 0. --tag "${seed}_${dataset}_${arch}_running_ensb" --parent_id "child_of_${seed}_${dataset}_${arch}_${pooling}_heads_ztw" --heads all &
    if ! (( (head_id + 1) % 2)); then
        wait
    fi
done
