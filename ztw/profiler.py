...
from architectures.SDNs.VGG_SDN import FcBlockWOutput as VggInternalClassifier
...
def count_adaptive_maxpool(m, x, y):
...
def count_adaptive_avgpool(m, x, y):
...
def profile_sdn(model, input_size, device):
    ...
    for layer_id, m in enumerate(model.modules()):
        if isinstance(m, (af.InternalClassifier, VggInternalClassifier)):
