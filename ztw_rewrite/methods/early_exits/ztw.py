...
class ZTWEnsembling(EarlyExitingBase):
    def __init__(self, base_model: nn.Module, type: str = 'geometric'):
        super().__init__()
        self._base_model = base_model
        self._ensembles = nn.ModuleList()
        self._type = type
        # TODO does ZTW also builds an ensemble on the original (backbone model's) head??

        for i in range(self._base_model.number_of_attached_heads):
            # for i in range(self._base_model.number_of_heads):
    ...
    #     return self.number_of_attached_heads
