...
# TODO perhaps use the torchvision model instead
...
class WideResNet(nn.Module):
    def __init__(self, num_blocks, widen_factor, num_classes, dropout_rate, stride_on_beginning=True):
        super().__init__()
        ...
        if stride_on_beginning is False:
            self.initial_conv = nn.Conv2d(3, 16, kernel_size=3, stride=1, padding=1, bias=True)
        else:
            self.initial_conv = nn.Conv2d(3, 16, kernel_size=3, stride=2, padding=1, bias=True)
        self.layers = nn.ModuleList()
        self.in_channels = 16
