...
if [ -f environment.yml ]; then
  ...
  TMPDIR=pip-build pip --no-input --no-cache-dir install wandb==0.15.3 tensorboard seaborn
  TMPDIR=pip-build pip --no-input --no-cache-dir install transformers datasets
  # TORCH_CUDA_ARCH_LIST="7.5+PTX 8.6" TMPDIR=pip-build pip --no-input --no-cache-dir install .
  rm -rf pip-build
