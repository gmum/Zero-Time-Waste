...
def in_training_eval(args, tc):
    if tc.state.current_batch in tc.eval_batch_list:
        ...
        test_loss, test_acc, test_f1 = test_classification(tc.accelerator,
                                                  tc.model,
                                                  tc.test_loader,
                                                  tc.criterion_type,
                                                  batches=args.eval_batches)
        train_loss, train_acc, train_f1 = test_classification(tc.accelerator,
                                                    tc.model,
        ...
        if tc.accelerator.is_main_process:
            ...
            tc.writer.add_scalar('Eval/Test F1', test_acc, global_step=tc.state.current_batch)
            ...
            tc.writer.add_scalar('Eval/Train F1', train_acc, global_step=tc.state.current_batch)
...
def final_eval(args, tc):
    if not tc.final_path.exists():
        if tc.accelerator.is_main_process:
            save_state(tc.accelerator, tc.state_path)
        # test on testset
        test_loss, test_acc, test_f1 = test_classification(tc.accelerator, tc.model, tc.test_loader, tc.criterion_type)
        if tc.accelerator.is_main_process:
            final_results = {}
            ...
            final_results['final_f1'] = test_f1
