...
def in_training_eval(args, tc):
    if tc.state.current_batch in tc.eval_batch_list:
        ...
        test_losses, test_accs, test_f1s = test_earlyexiting_classification(tc.accelerator,
                                                                  tc.model,
                                                                  tc.test_loader,
                                                                  tc.criterion_type,
                                                                  batches=args.eval_batches)
        train_losses, train_accs, train_f1s = test_earlyexiting_classification(tc.accelerator,
                                                                    tc.model,
        ...
        if tc.accelerator.is_main_process:
            for i in range(tc.accelerator.unwrap_model(tc.model).number_of_heads):
                ...
                tc.writer.add_scalar(f'Eval/Head {i} test F1', test_f1s[i], global_step=tc.state.current_batch)
                ...
                tc.writer.add_scalar(f'Eval/Head {i} train F1', train_f1s[i], global_step=tc.state.current_batch)
