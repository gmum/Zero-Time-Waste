...
def main():
    ...
    # ════════════════════════ common EE runs ════════════════════════ #
    last_head = 40
    place_every_variant = [1, 2, 3, 4, 5, 10]
    place_at = {}
    for place_every in place_every_variant:
        place_at[place_every] = [head_slot for head_slot in range(last_head) if (head_slot - 1) % place_every == 0]
    ...
    for place_every in place_every_variant:
        for exp_id in exp_ids:
            ...
            args.exp_id = exp_id
            args.model_args.place_at = place_at[place_every]
            ...
            run_to_job_map[run_name] = job
        exp_names.append(exp_name)
        display_names.append(f'SDN every={place_every}')
    ...
    # # ════════════════════════ train GPF models ════════════════════════ #
    #
    # for place_every in place_every_variant[1:]:
    #     for lr, wd in [
    #         # (0.0005, 0.0),
    #         # (0.0001, 0.0),
    #         # (0.0001, 0.1),
    ...
    #             run_to_job_map[run_name] = job
    #         exp_names.append(exp_name)
    #         display_names.append(f'GPF every={place_every}')
    #         # display_names.append(f'GPF lr={lr}')
    #
    # # ════════════════════════ L2W model settings ════════════════════════ #
    #
    ...
    #             run_to_job_map[run_name] = job
    #         exp_names.append(exp_name)
    #         display_names.append(f'L2W every={place_every}')
    ...
    for place_every in place_every_variant:
        ...
        for optimizer, lr, wd in [
            ...
            display_names.append(f'ZTW every={place_every}')
    ...
    out_dir_name = f'ztw_efficientnet_v2_{common_args.dataset}_number_of_heads'
