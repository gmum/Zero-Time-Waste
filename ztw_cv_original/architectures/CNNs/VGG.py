...
class ConvBlock(nn.Module):
    def __init__(self, args, conv_params):
        ...
        conv_layers = []
        conv_layers.append(nn.Conv2d(in_channels=input_channels, out_channels=output_channels, kernel_size=3,
                                     padding=1))
...
class FcBlock(nn.Module):
    def __init__(self, args, fc_params, flatten):
...
class VGG(nn.Module):
    def __init__(self, args, params):
        ...
        for layer_id, channel in enumerate(self.conv_channels):
            ...
            self.layers.append(ConvBlock(args, conv_params))
        ...
        for layer_id, width in enumerate(self.fc_layer_sizes[:-1]):
            ...
            self.layers.append(FcBlock(args, fc_params, flatten=flatten))
            fc_input_size = width
